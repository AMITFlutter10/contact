...
// register email& password
 ...
 late XFile image;
  ...
  registerByEmailAndPassword({ required String email,
     required String password, String? name}) async {
    try{
    ...
    await storage.ref().child("images/").child("${userModel.id}.png}")
        .putFile(File(userImage!.path));
    userModel.pic=  await storage.ref().child("images/").child("${userModel.id}.png}")
        .getDownloadURL();
    await store.collection("Users").doc(userModel.id).
    set(userModel.toMap());
